...
export async function listChats(userId: string) {
  ...
  return chats.map(c => ({ id: c._id.toString(), title: c.title, createdAt: c.createdAt }));
}
...
export async function listMessages(userId: string, chatId: string) {
  const chat = await Chat.findOne({ _id: chatId, userId });
  if (!chat) throw new ApiError(404, 'Chat not found');
  const msgs = await Message.find({ chatId: chat._id }).sort({ createdAt: 1 }).lean();
  return msgs.map(m => ({
    id: m._id.toString(),
    role: m.role,
    content: m.content,
...
export async function addUserAndAssistantMessage(userId: string, chatId: string, content: string) {
  ...
  const cachedUser = await Message.findOne({
    ...
    .lean();
  if (cachedUser) {
    const cachedAssistant = await Message
      .findOne({ chatId: cachedUser.chatId, role: 'assistant', createdAt: { $gte: cachedUser.createdAt } })
      .sort({ createdAt: 1 })
      .lean();
...
export async function editAndResend(userId: string, chatId: string, userMessageId: string, content: string) {
  ...
  if (!lastUser || lastUser._id.toString() !== userMsg._id.toString()) {
  ...
  if (nextAssistant) {
    await Message.deleteOne({ _id: nextAssistant._id });
  }
...
export async function deleteSingleMessage(userId: string, chatId: string, messageId: string) {
  ...
  if (msg.role === 'user') {
    ...
    if (prevUser) {
      await Message.deleteOne({ _id: prevUser._id });
    }
  }
